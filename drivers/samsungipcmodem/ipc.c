...
static gboolean call_blocking_read(gpointer user_data)
{
	...
	while (can_write_data(device->io, G_IO_OUT, device) == TRUE);
	write_watch_destroy(device);
}
