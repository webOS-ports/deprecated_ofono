										...
										void *user_data)
{
	...
	snprintf(gateway, IP_MAX_LENGTH, "%i.%i.%i.%i", resp->gateway[0], resp->gateway[1],
			 resp->gateway[2], resp->gateway[3]);
	snprintf(subnetmask, IP_MAX_LENGTH, "%i.%i.%i.%i", resp->subnet_mask[0], resp->subnet_mask[1],
			 resp->subnet_mask[2], resp->subnet_mask[3]);

	ofono_debug("local = %s, dns1 = %s, dns2 = %s, gateway = %s, subnetmask = %s", local, dns1, dns2, gateway, subnetmask);
	...
	ipc_device_remove_watch(gcd->device, gcd->ip_conf_watch);
...
static void pdp_context_activate_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	gcd->ip_conf_watch = ipc_device_add_notifcation_watch(gcd->device, IPC_GPRS_IP_CONFIGURATION,
...
static void define_pdp_context_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	ipc_gprs_pdp_context_setup(req, gcd->active_context, 1, /* enable context */
						gcd->username, gcd->password);

	if(ipc_device_enqueue_message(gcd->device, IPC_GPRS_PDP_CONTEXT, IPC_TYPE_SET,
				...
				ofono_gprs_context_cb_t cb, void *data)
{
	...
	ipc_gprs_define_pdp_context_setup(req, ctx->cid, 1, ctx->apn);
					...
					ofono_gprs_context_cb_t cb, void *data)
{
	...
	ipc_gprs_pdp_context_setup(req, cid, 0, /* deactivate the context */
					NULL, NULL);
					...
					unsigned int vendor, void *data)
{
	...
	gcd->call_status_watch = ipc_device_add_notifcation_watch(gcd->device, IPC_GPRS_CALL_STATUS,
...
static void samsungipc_gprs_context_remove(struct ofono_gprs_context *gc)
{
	...
	if (gcd->call_status_watch > 0)
		ipc_device_remove_watch(gcd->device, gcd->call_status_watch);
